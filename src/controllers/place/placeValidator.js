...
const placeValidator = Joi.object({
    ...
    events: Joi.array().items(Joi.string().pattern(objectIdPattern).error(new Error('Invalid place event ObjectId.'))).optional().error(new Error('Invalid place events array.')),
